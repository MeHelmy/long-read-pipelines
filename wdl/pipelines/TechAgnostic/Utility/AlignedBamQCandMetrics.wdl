...
workflow Work {
    ...
    parameter_meta {
        ...
        # inputs
        bam_descriptor: "A description of the purpose of the BAM (doesn't need to be single-file specific)."
        tech: "The technology used to generate this BAM. Currently, the following values are accepted: [ONT, Sequel, Revio]."

        gcs_out_root_dir: "output files will be copied over there"

        cov_bed: "An optional BED file on which coverage will be collected (a mean value for each interval)"
        cov_bed_descriptor: "A short description of the BED provided for targeted coverage estimation; will be used in naming output files."

        fingerprint_store:  "A GCS 'folder' holding fingerprint VCF files"
        sample_id_at_store: "The ID of the sample supposedly this BAM belongs to; note that the fingerprint VCF is assumed to be located at {fingerprint_store}/{sample_id_at_store}*.vcf(.gz?)"

        vbid2_config_json: "A config json to for running the VBID2 contamination estimation sub-workflow; if provided, will trigger the VBID2 sub-workflow."

        check_postaln_methyl_tags: "If true, will run a sub-workflow to collect methylation tags information."

        # outputs
        wgs_cov:                    "whole genome mean coverage"
        coverage_per_chr:           "a file summarizing coverage per chromosome"

        nanoplot_summ:              "Summary on alignment metrics provided by Nanoplot (todo: study the value of this output)"

        sam_flag_stats:             "SAM flag stats as a WDL Map"

        inferred_sex_info:          "Inferred sex concordance information if expected sex info is provided"
        methyl_tag_simple_stats:    "Simple stats on the reads with & without methylation SAM tags."

        aBAM_metrics_files_out:     "A map where keys are summary-names and values are paths to files generated from the various QC/metrics tasks"
    }
    ...
    # object below is commented out, for womtool validation to pass, but miniwdl demands this object keyword to pass its own check
    # (see https://github.com/chanzuckerberg/miniwdl/issues/256)
